...
import { Plus, Search, ListRestart } from "lucide-react"
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="w-full lg:w-1/4">
          <div className="bg-white rounded-lg shadow p-4 h-full">
            <h2 className="text-lg font-semibold text-gray-800 mb-4 border-b pb-2">Create New Admin</h2>
            <div className="space-y-4">
              ...
              <div>
                <label
                  className={`text-sm block mb-1 ${
                    focusedField === "department" ? "text-blue-600 font-medium" : "text-gray-600"
                  }`}
                >
                  Assign Department
                </label>
                <select
                  ...
                  className={`w-full border rounded-xl px-3 py-2 focus:outline-none focus:ring-2 bg-white ${
                    focusedField === "department" ? "border-blue-500 ring-blue-500" : "border-gray-300"
                  }`}
                >
              ...
              {formData.departmentCodes.map((code, index) => {
                ...
                return (
                  <div key={index}>
                    <label
                      className={`text-sm block mb-1 ${
                        focusedField === fieldName ? "text-blue-600 font-medium" : "text-gray-600"
                      }`}
                    >
                    ...
                    <input
                      type="text"
                      value={code}
                      onChange={(e) => handleDepartmentCodeChange(index, e.target.value)}
                      onFocus={() => setFocusedField(fieldName)}
                      onBlur={() => setFocusedField(null)}
                      className={`w-full border rounded-xl px-3 py-2 focus:outline-none focus:ring-2 ${
                        focusedField === fieldName ? "border-blue-500 ring-blue-500" : "border-gray-300"
                      }`}
                    />
                  </div>
                )
